...
// viewportMargin: Specifies the amount of lines that are rendered above and below the part of the document that's currently scrolled into view
...
'    document.write(s);\n  }\n  document.writeln("<br>");\n}\n</script>\n</p>\n';
     ...
     load_code_and_run('samples', 'hello-world');
...
// kkk
  ...
  $('#js_username_uri').html(user_uri); // url link base: https://js.do/user_uri/[_____]
...
//      document.location.href = $js.js_url_REQUEST_URI;
    ...
    var username     = uri[0];
    var js_permalink = uri[1];

    load_code_and_run(username, js_permalink);
    return 1;
  }
...
all_code = '<a class="close" href="https://js.do/">&times;</a>'+all_code;
     ...
     'callback':
      function(res) {
        ...
        if (js_id) { // code was found
          ...
          if (js_username == 'samples' && js_permalink == 'hello-world') {
            // Load sample (homepage), will not update path/filename. Keep similar to new blank newcode.
          } else {
...
function hide_user_info(user_username) { // called on logout
  ...
  if ($('#js_permalink').val()=='') {
    var  user_uri = 'code';
    $('#js_username_uri').html(user_uri); // url link base: https://js.do/code/
  }
